...
abstract class Paginator
{
    ...
    protected function __construct($items, $listRows, $currentPage = null, $total = null, $simple = false, $options = [])
    {
        ...
        $this->options['list_rows'] = $listRows;
        if ($simple) {
    ...
    protected function url($page)
    {
        ...
        $pathStr = strstr($this->options['path'],'.html') ? substr($this->options['path'], 0, -5).'_html' : $this->options['path'];
        foreach ($parameters as $k => $v) {
            unset($parameters['/'.$pathStr]);
            unset($parameters[$pathStr]);
        }
        $url = $path;
        if (!empty($parameters)) {
            $url = '?' . ltrim(urldecode(http_build_query($parameters, null, '&')),'/');
        }
